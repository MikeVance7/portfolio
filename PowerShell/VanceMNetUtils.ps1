...
Function Get-IPNetwork {
    ...
    if ($cidr[1]) {
        ...
        try {
            if ([int]$cidr[1] -lt 0 -or [int]$cidr[1] -gt 32) {
                Write-Host 'CIDR notation must be between 0 and 32'
                return
            }
        }
        catch {
    ...
    if ($SubMask) {
        ...
        if ($findClass[0] -ge 224) { Write-Output 'Class D and E Addresses are experimental'; exit }
        $net = [IPAddress]($ip.Address -band $mask.Address)
        Write-Output $net
    }
}
